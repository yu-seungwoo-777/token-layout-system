...
- **(B)** Claude Design `.dc.html`이 소스면 → `node assets/scripts/extract-dc.mjs <file> --out src/styles/tokens`를 실행해 DC 스케일 + 시맨틱 변수에서 4계층 CSS를 생성합니다(매핑·배선·DC가 커버하지 않는 부분 — 예: 에러 색상 — 은 [`references/dc-to-tokens.md`](references/dc-to-tokens.md) 참조).
- **(C)** 그 외(Figma / JSON / 소스 없음) → `assets/tokens/`의 기본 CSS 파일 4개를 `src/styles/tokens/`로 복사합니다:
...
- `assets/scripts/extract-dc.mjs` — DC `.dc.html` → 4계층 토큰 변환기(Step 1, branch B). `__fixtures__/mini.dc.html` + `extract-dc.test.mjs` 계약 테스트 포함 (`node --test assets/scripts/extract-dc.test.mjs`)
