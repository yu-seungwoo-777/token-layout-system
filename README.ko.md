...
**추출이 어려워지면** 두 가지 중 하나이고, 각각에 대한 프로토콜이 문서화돼 있습니다 — *드러내고, 조용히 넘기지 않기*: **(a) 준비된 내용 부족**(소스가 필요한 역할에 값을 지정하지 않음 — 플래그 + 원칙적 디폴트로 채우거나, 다크모드처럼 제품 결정이면 stop-and-ask) vs **(b) 인텐트 모호성**(소스가 역할을 불명확하게 채움 — 후보가 여럿이거나 자기모순 — 선택지를 제시해 stop-and-ask 후 재생성). [`references/dc-to-tokens.md`](references/dc-to-tokens.md) → *When extraction gets hard* 참조. 산출물에 묻힌 합리적 선택도 silent design decision입니다.
...
- `assets/scripts/verify-tokens.mjs` — **포맷 독립적** 토큰 검증기(모든 소스): 단절 `var()` 참조, Typography 의존, dark-pair 완전성, WCAG(최선: hex/rgb만). `verify-tokens.test.mjs` 계약 테스트 (`node --test assets/scripts/verify-tokens.test.mjs`).
- `references/workflow.md` — 7단계 빌드 워크플로의 상세 (각 단계 진입 시점에 읽기)
- `references/dc-to-tokens.md` — 디자인 소스 → 4계층 **매핑 원리**(모든 소스용), `.dc.html` 참고 어댑터, SPA 번들 안내, 추출이 어려울 때의 프로토콜(준비된 내용 부족 vs 인텐트 모호성). Step 1 branch B용으로 읽을 것.
