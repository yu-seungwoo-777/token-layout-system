...
Next.js(App Router) + Tailwind v4 CSS-first `@theme`로 재사용 가능한 토큰 기반 레이아웃 시스템을 만드는 [Claude Code 스킬](https://docs.claude.com/en/docs/claude-code/skills)입니다.

4계층 CSS 토큰 시스템(raw → semantic → layout → component), 1/2/3단 컬럼과 반응형 사이드바→드로어를 전환할 수 있는 `Shell` 컴포넌트, 그리고 모든 색상·치수가 하나의 토큰 소스에서 나오도록 retrofit된 라이브러리 컴포넌트를 만들어 냅니다 — 다크 모드는 컴포넌트 코드 수정 없이 동작합니다.

아키텍처는 컴포넌트 라이브러리에 독립적입니다. **shadcn/ui는 워크플로우와 함정들을 검증한 작업 예시(worked example)**이고, 시스템의 유일한 shadcn 하드 의존성은 `Shell`의 모바일 드로어(`Sheet`)입니다 — 그 import 하나만 교체하면 같은 retrofit 방법이 다른 라이브러리 산출물에도 그대로 적용됩니다.

## 왜 프롬프트가 아니라 스킬인가

강한 모델은 스킬 없이도 토큰 레이아웃을 만들 수 있습니다. 스킬이 사주는 것은 — 자체 [벤치마크](#벤치마크)가 보여주듯 — 세 곳에 집중되어 있습니다:

1. **매번 동일한 구조.** 스킬 없는 실행은 세션마다 다른 토큰 구성을 즉흥합니다(대개 한 파일에 인라인). 스킬은 4계층 구조를 고정하므로 손대는 모든 프로젝트가 동일하게 조직됩니다 — 벤치마크에서 가장 뚜렷한 차이는 특정 기법이 아니라 이 일관성이었습니다.
2. **어떤 정적 게이트도 못 잡는, 선불된 런타임 지식.** [함정 12가지](#시작-전에-알아둘-함정-12가지)는 전부 `grep`·`tsc`·`next build`를 통과하고 런타임이나 육안에서만 드러납니다 — 각각 한 번의 실제 디버깅 시간으로 지불됐고, 여러분은 다시 지불하지 않습니다.
3. **실제로 실행되는 검증.** 모델은 Playwright 스펙을 작성하고 실행하지 않는 경우가 흔합니다. 이 스킬은 실행되지 않은 테스트를 실패 상태로 취급하고, 런타임 게이트는 프로덕션 빌드를 대상으로 합니다.

사주지 **않는** 것도 명시합니다: 일반적인 "검증을 더 추가하는" 엔지니어링. 후속 평가에서 강한 baseline은 실패 모드가 설명되기만 하면 동등한 sanity check를 스스로 만들었습니다 — 스킬의 가치는 관례와 함정이지, 범용적 엄밀함이 아닙니다.

**비적용 대상:** 기존 토큰/디자인 시스템이 있는 프로젝트(4계층 구조와 충돌), Tailwind v3나 `@theme` 비사용 환경, Next.js App Router 외 스택, 3단 verify 파이프라인이 과한 일회성 프로토타입.

## 자산 계약 — 무엇이 고정이고 무엇이 여러분 것인가

`assets/`는 전부 같은 성격의 파일이 아닙니다. 각 파일은 세 등급 중 하나에 속합니다:

| 등급 | 파일 | 고정(계약) | 자유(여러분 것) |
|---|---|---|---|
| **구조 (계약)** | `tokens/*.css`, `globals.css` | 4계층 형태, 레이어 건너뛰기 금지, `@theme inline` 자기참조 배선, import 순서 | 모든 *값* — 색상, 스케일, 치수 |
| **스캐폴드 (복사 후 소유)** | `components/layout/*`, `typography.tsx`, Playwright 설정+스펙 | grep 가드 통과 유지 | 그 외 전부 — 포크해서 소유하세요, 업데이트를 기대하는 라이브러리가 아닙니다 |
| **방법 (파일 미배포)** | shadcn/라이브러리 컴포넌트 | retrofit 루프(`references/shadcn-retrofit.md`) + grep 후 수정 | 어떤 라이브러리·어떤 컴포넌트를 쓸지 — 매번 새로 생성 |

`assets/`의 어떤 것도 업데이트받는 의존성이 아닙니다 — 소유권을 가져가는 출발점입니다. 협상 불가한 것은 토큰 *구조*와 grep 규칙뿐이고, 그 둘이 verify 파이프라인이 강제하는 대상입니다.
...
`globals.css`에 `@theme inline`으로 배선합니다 — 이 자기 참조 패턴(`--color-primary: var(--color-primary)`)이 다크모드를 담당합니다. 순환 참조라 computed-value time에 invalid가 되어, 실제 값은 `:root`/`.dark`에서 소스 순서로 결정됩니다 (함정 #1에 검증된 메커니즘이 있고, 이 모양에서는 `inline` 키워드 자체는 장식적입니다).
...
**4. Atomic 컴포넌트.** `shadcn add button input badge card`(또는 선택한 라이브러리에서 동등한 컴포넌트를 생성)를 실행한 뒤 [`references/shadcn-retrofit.md`](references/shadcn-retrofit.md)의 before/after 표대로 각각 retrofit합니다 — 갓 생성된 shadcn 출력물은 grep 가드를 즉시 위반하므로(`ring-[3px]`, `rounded-[min(var(--radius-md),10px)]` 등) 이 단계는 선택적 클린업이 아닙니다. `Typography`도 추가합니다(shadcn 기본 미제공).
...
`src/components/**`에 raw `px`나 `#hex`를 넣지 않습니다. 모든 치수는 토큰 참조입니다 — `var(--token)`, `[var(--token)]` 형태의 arbitrary, 또는 소스에 리터럴 없이 rem으로 컴파일되는 Tailwind 스케일 유틸리티. 토큰 *정의* 파일(`src/styles/**`)만 예외입니다 — 그게 raw 레이어의 존재 이유입니다. `scripts/verify.sh`가 `src/components` 전체(그리고 `src/app`)를 대상으로 `grep -rE "[0-9]+px|#[0-9a-fA-F]{3,8}\b|rgba?\(|hsla?\(|oklch\(|oklab\(|lab\(|lch\(|color\(" src/components`로 이를 강제합니다. px·3~8자리 hex·CSS 색상 함수 리터럴은 잡지만, 단위 없는 매직 넘버(z-index, opacity 등)는 잡지 못하므로 사람이 직접 확인해야 합니다. 파이프라인은 살아있는 프로젝트에서의 자기 부패도 방어합니다 — 레이어 규칙(컴포넌트의 raw 팔레트 토큰 직접 참조 금지), 비차단 드리프트 경고(일회성 rem/vh arbitrary, 숫자 인라인 스타일), 그리고 스펙이 방문하지 않는 페이지가 존재하면 실패하는 스모크 커버리지 패리티 테스트. 부패 모드 자체는 [`references/gotchas.md`](references/gotchas.md) 말미에 정리되어 있습니다.
